...
:: Shortcuts to rewrite (notice not including the year bc it changes every year)
...
:GenerateShortcut
:: if application .exe icon exist, generate new shortcut with default embedded icon
if exist "%adobefolder%\%~1\%~2.exe" (
	powershell "$s=(New-Object -COM WScript.Shell).CreateShortcut('%programdata%\Microsoft\Windows\Start Menu\Programs\%~1.lnk');$s.TargetPath='%adobefolder%\%~1\%~2.exe';$s.IconLocation='%adobefolder%\%~1\%~2.exe,0';$s.Save()"
...
for /f "tokens=2 delims= " %%A in ('echo %~2') do (
	if exist "%adobefolder%\%~1\%%~A.exe" (
		powershell "$s=(New-Object -COM WScript.Shell).CreateShortcut('%programdata%\Microsoft\Windows\Start Menu\Programs\%~1.lnk');$s.TargetPath='%adobefolder%\%~1\%%~A.exe';$s.IconLocation='%adobefolder%\%~1\%%~A.exe,0';$s.Save()"
		if !errorlevel!==0 echo %~1 - ok
		goto :eof
	)
)
